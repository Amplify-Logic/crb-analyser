...
export default function ReportViewer() {
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'findings' | 'recommendations' | 'roadmap'>('summary')
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => window.open(`/api/reports/public/${report.id}/pdf`, '_blank')}
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-br from-primary-600 to-purple-700 rounded-2xl p-8 text-white mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <p className="text-primary-200 text-sm mb-1">AI Readiness Score</p>
              <div className="text-6xl font-bold mb-1">{summary?.ai_readiness_score || 0}</div>
              <p className="text-primary-200 text-xs">out of 100</p>
            </div>
            <div className="text-center border-l border-r border-white/20 px-4">
              <p className="text-primary-200 text-sm mb-2">Two Pillars</p>
              <div className="flex justify-center gap-4">
                <div>
                  <div className="text-3xl font-bold">{summary?.customer_value_score || 0}</div>
                  <p className="text-xs text-primary-200">Customer Value</p>
                </div>
                <div>
                  <div className="text-3xl font-bold">{summary?.business_health_score || 0}</div>
                  <p className="text-xs text-primary-200">Business Health</p>
                </div>
              </div>
            </div>
            <div className="text-center">
              <p className="text-primary-200 text-sm mb-1">3-Year Value Potential</p>
              <div className="text-2xl font-bold text-green-300">
                {formatCurrency(summary?.total_value_potential?.min || 0)} - {formatCurrency(summary?.total_value_potential?.max || 0)}
              </div>
            </div>
          </div>
          ...
          {summary?.key_insight && (
            <div className="mt-6 pt-6 border-t border-white/20">
              <p className="text-sm text-primary-200 mb-1">Key Insight</p>
              <p className="text-lg">{summary.key_insight}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-8">
          <h3 className="text-lg font-semibold text-green-800 mb-4">Value Summary (3-Year Projection)</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl p-4">
              <p className="text-sm text-gray-500 mb-1">Value SAVED (Efficiency)</p>
              <p className="text-xl font-bold text-green-600">
                {formatCurrency(value_summary?.value_saved?.subtotal?.min || 0)} - {formatCurrency(value_summary?.value_saved?.subtotal?.max || 0)}
              </p>
            </div>
            <div className="bg-white rounded-xl p-4">
              <p className="text-sm text-gray-500 mb-1">Value CREATED (Growth)</p>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {(['summary', 'findings', 'recommendations', 'roadmap'] as const).map((tab) => (
            <button
              key={tab}
          ...
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  {summary.top_opportunities.map((opp, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-green-50 rounded-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                <div className="text-3xl font-bold text-primary-600">{findings?.length || 0}</div>
                <p className="text-sm text-gray-500">Findings</p>
              </div>
              <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                <div className="text-3xl font-bold text-green-600">{recommendations?.length || 0}</div>
        ...
        {activeTab === 'findings' && (
          <div className="space-y-4">
            {findings?.map((finding) => (
              <div key={finding.id} className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div key={rec.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <svg
                      className={`w-6 h-6 text-gray-400 transition-transform ${expandedRec === rec.id ? 'rotate-180' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                {expandedRec === rec.id && (
                  <div className="px-6 pb-6 border-t">
                      ...
                      <div className="mt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {rec.options.off_the_shelf && (
                            <div className={`p-4 rounded-xl border-2 ${rec.our_recommendation === 'off_the_shelf' ? 'border-primary-500 bg-primary-50' : 'border-blue-200 bg-blue-50'}`}>
                          ...
                          {rec.options.best_in_class && (
                            <div className={`p-4 rounded-xl border-2 ${rec.our_recommendation === 'best_in_class' ? 'border-primary-500 bg-primary-50' : 'border-green-200 bg-green-50'}`}>
                          ...
                          {rec.options.custom_solution && (
                            <div className={`p-4 rounded-xl border-2 ${rec.our_recommendation === 'custom_solution' ? 'border-primary-500 bg-primary-50' : 'border-purple-200 bg-purple-50'}`}>
                    ...
                    {/* Our Recommendation */}
                    {rec.our_recommendation && (
                      <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
                        <p className="font-semibold text-green-800">Our Recommendation: {rec.our_recommendation.replace('_', '-')}</p>
                        <p className="text-sm text-gray-600 mt-1">{rec.recommendation_rationale}</p>
                      </div>
                    )}

                    {/* ROI & Payback */}
                    {(rec.roi_percentage || rec.payback_months) && (
                      <div className="mt-4 flex gap-4">
                        {rec.roi_percentage && (
                          <div className="px-4 py-2 bg-primary-600 text-white rounded-lg">
                            <span className="font-bold">{rec.roi_percentage}% ROI</span>
                          </div>
                        )}
                        {rec.payback_months && (
                          <div className="px-4 py-2 bg-gray-100 text-gray-800 rounded-lg">
                            <span className="font-bold">{rec.payback_months} month payback</span>
                          </div>
                        )}
                      </div>
                    )}
                ...
                <div className="space-y-3">
                  {roadmap.short_term.map((item, i) => (
                    <div key={i} className="border-l-4 border-green-400 pl-4 py-2">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Timeline: {item.timeline} | Expected: {item.expected_outcome}</p>
                    </div>
                  ))}
                </div>
              </div>
