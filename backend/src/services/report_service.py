...
class ReportGenerator:
    ...
    async def generate_report(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            # Build readiness profile early for use across all phases
            from src.services.readiness_profile import build_readiness_profile as build_rp
            readiness_profile = build_rp(self.context.get("answers", {}))
            self._partial_data["readiness_profile"] = readiness_profile
            logger.info("readiness_profile_built", **readiness_profile)
            ...
            education_path = system_architecture.get("education_path", [])
            recommended_skills = system_architecture.get("recommended_skills", [])
            context_os_template = system_architecture.get("context_os_template")
            self._partial_data["education_path"] = education_path
            self._partial_data["recommended_skills"] = recommended_skills
            self._partial_data["context_os_template"] = context_os_template

            # Enrich playbooks with skill-building tasks from AIOS analysis
            if recommended_skills and playbooks:
                playbooks = self._enrich_playbooks_with_skills(playbooks, recommended_skills)
                self._partial_data["playbooks"] = playbooks
            ...
            try:
                ...
                if aios_maturity:
                    update_data["aios_maturity"] = aios_maturity
                # Include readiness profile from quiz/workshop data
                if readiness_profile:
                    update_data["readiness_profile"] = readiness_profile
...
Return ONLY the JSON."""
    ...
    def _enrich_playbooks_with_skills(
        self,
        playbooks: List[Dict[str, Any]],
        recommended_skills: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Append skill-building tasks to playbooks from AIOS analysis.

        Adds a 'skill_building_tasks' section to each playbook with
        relevant skills from the system architecture recommendation.
        """
        if not recommended_skills:
            return playbooks

        for playbook in playbooks:
            # Match skills to playbook by checking tool overlap
            playbook_tools = set()
            for phase in playbook.get("phases", []):
                for week in phase.get("weeks", []):
                    for task in week.get("tasks", []):
                        task_name = task.get("name", "").lower()
                        playbook_tools.add(task_name)
            ...
            for skill in recommended_skills[:3]:
                matching_skills.append({
                    ...
                    "time_saved_monthly_hours": skill.get("estimated_time_saved_monthly", 0),
                })

            if matching_skills:
                playbook["skill_building_tasks"] = matching_skills
    ...
    def _generate_system_architecture(self, recommendations: List[Dict]) -> Dict[str, Any]:
        ...
        if skill:
            try:
                context = self._get_skill_context()
                # Ensure recommendations are available in report_data
                if context.report_data is None:
                    context.report_data = {}
                ...
                if result.success:
                    ...
                    if "maturity" in arch_data:
                        self._partial_data["aios_maturity"] = arch_data["maturity"]
                    logger.info(
                        "system_architecture_generated",
                        skill="system-architecture",
                        maturity=arch_data.get("maturity", {}).get("current_level"),
                        layers=len(arch_data.get("layers", {}).get("stack", [])),
                    )
                    return arch_data
                else:
                    logger.warning("system_architecture_skill_failed", warnings=result.warnings)
            except Exception as e:
                logger.warning("system_architecture_skill_error", error=str(e))

        # Fallback to legacy 3-column architecture
