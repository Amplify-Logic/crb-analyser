...
# "test the quiz flow"                    → -s=quiz-test
# "scrape pricing from vendor.com"        → -s=vendor-pricing
# "UI test the report viewer"             → -s=report-viewer-test

playwright-cli -s=quiz-test open http://localhost:5174/quiz --persistent
playwright-cli -s=quiz-test snapshot
playwright-cli -s=quiz-test click e12
```
...
| Report Viewer | `http://localhost:5174/report/{id}` |
| Checkout | `http://localhost:5174/checkout/{session_id}` |
| Admin Dashboard | `http://localhost:5174/admin` |
| Backend Health | `http://localhost:8383/health` |
